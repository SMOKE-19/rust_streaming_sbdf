...
- `streaming_sbdf_rs` 자체는 저수준 SBDF writer 패키지입니다.
- parquet 파일을 직접 읽는 것은 상위 helper인 `smoking_data.streaming_sbdf.parquet_to_sbdf_streaming(...)` 가 담당합니다.
- 상위 helper는 `Path` 입력을 받을 수 있습니다.
- 상위 helper에서 `column_types` 를 생략하면 DuckDB가 해석한 Parquet 스키마를 기준으로 Spotfire 타입을 자동 매핑합니다.
